...
let player = { x: 300, y: 280, w: 14, h: 20, speed: 2, facing: 'down', frame: 0 };
let flowers = [];
let score = 0;
let fountainDrops = [];
let stars = [];
...
function resumeFriskMusic() { if (!isFriskMusicPlaying && isGifShown && !isKrisPopupShown && !isGasterShown && !gameRunning) { audioFrisk.volume = currentVolume; audioFrisk.play().catch(() => {}); isFriskMusicPlaying = true; } }
...
function showAnswer(qNum) {
    ...
    const images = { 1: '1667059529_4-zefirka-club-p-fon-anderteil-zolotie-tsveti-4.jpg', 2: 'https://litter.catbox.moe/sob9v51fp9j28lok.webp', 3: 'fdfc8cdf655e9bfb0c069bc9b35ef675.jpg', 4: '636drhmtadud1.gif', 6: 'eb1e054e383b6da88f322d846e8d79ed.jpg', 7: '11df82aea021e2fc18ab681008d47bc4.jpg', 8: 'c2a0c65ceb743c3bd355cb01ac1eeb24.jpg', 9: '2d5238d4daea905cfb2c4c4c9feec2f1.jpg', 10: '37125d9c99120d3c1a01df4473f2e0f8.jpg' };
...
function hideSecretMessage() { stopSecretGlitches(); stopMysteryAudio(); secretBox.classList.add('fade-out'); setTimeout(() => { secretBox.style.display = 'none'; secretBox.classList.remove('fade-out','visible'); menuBox.classList.remove('hidden'); isSecretShown = false; }, 300); }
...
function spawnFlowers(count = 8) { flowers = []; for (let i = 0; i < count; i++) { flowers.push({ x: 30 + Math.random() * 540, y: 210 + Math.random() * 100, size: 6 + Math.random() * 5, sway: Math.random() * Math.PI * 2 }); } }
...
function startMiniGame() {
    ...
    pauseFriskMusic();
    questionsWindow.classList.remove('visible'); questionsWindow.classList.add('fading'); isQuestionsShown = false;
    gameCanvas.style.display = 'block'; gameScore.style.display = 'block';
    gameCanvas.width = 600; gameCanvas.height = 380;
    gameScore.textContent = '💛 x 0'; gameFrame = 0;
...
function stopMiniGame() {
    ...
    if (isGifShown && !isKrisPopupShown && !isGasterShown) resumeFriskMusic();
    questionsWindow.classList.add('visible'); questionsWindow.classList.remove('fading'); isQuestionsShown = true;
    gameCanvas.style.display = 'none'; gameScore.style.display = 'none';
}

function drawPixelChar(x, y, facing, frame) {
    gctx.imageSmoothingEnabled = false;
    if (facing === 'down') { gctx.fillStyle = '#ff5555'; gctx.fillRect(x+3,y+6,8,14); gctx.fillStyle = '#ffddbb'; gctx.fillRect(x+3,y,8,8); gctx.fillStyle = '#000'; gctx.fillRect(x+4,y+2,2,2); gctx.fillRect(x+8,y+2,2,2); gctx.fillStyle = '#333399'; if(frame%30<15){gctx.fillRect(x+3,y+20,3,4);gctx.fillRect(x+8,y+20,3,4)}else{gctx.fillRect(x+3,y+20,4,3);gctx.fillRect(x+7,y+20,4,3)} }
    else if (facing === 'up') { gctx.fillStyle = '#ff5555'; gctx.fillRect(x+3,y+6,8,14); gctx.fillStyle = '#553311'; gctx.fillRect(x+2,y,10,6); gctx.fillRect(x+4,y-2,6,3); gctx.fillStyle = '#333399'; if(frame%30<15){gctx.fillRect(x+3,y+20,3,4);gctx.fillRect(x+8,y+20,3,4)}else{gctx.fillRect(x+3,y+20,4,3);gctx.fillRect(x+7,y+20,4,3)} }
    else { const flip = facing==='right'?1:-1; gctx.fillStyle = '#ff5555'; gctx.fillRect(x+3,y+6,8,14); gctx.fillStyle = '#ffddbb'; gctx.fillRect(x+3,y,8,8); gctx.fillStyle = '#000'; gctx.fillRect(x+5+flip*2,y+2,2,2); gctx.fillStyle = '#333399'; if(frame%30<15){gctx.fillRect(x+4,y+20,3,4);gctx.fillRect(x+7,y+20,3,4)}else{gctx.fillRect(x+4,y+20,2,5);gctx.fillRect(x+8,y+20,2,5)} }
}

function drawPixelFlower(x, y, size, sway) { const sx = x+Math.sin(sway)*1.5; gctx.fillStyle='#228822'; gctx.fillRect(sx-1,y,2,size*1.5); gctx.fillStyle='#ffdd44'; for(let a=0;a<5;a++){const angle=a*Math.PI*2/5+gameFrame*0.02;gctx.fillRect(sx+Math.cos(angle)*size*0.7-2,y+Math.sin(angle)*size*0.5-2,4,4)} gctx.fillStyle='#ffaa00';gctx.fillRect(sx-2,y-2,4,4); }

function drawBigFountain() {
    gctx.fillStyle='#556677';gctx.fillRect(240,310,120,16);gctx.fillStyle='#667788';gctx.fillRect(250,308,100,4);gctx.fillStyle='#445566';gctx.fillRect(245,314,110,6);
    gctx.fillStyle='#334466';gctx.fillRect(248,318,104,12);gctx.fillStyle='#5588aa';gctx.fillRect(252,320,96,6);gctx.fillStyle='#77aacc';gctx.fillRect(256,322,88,3);
    gctx.fillStyle='#889999';gctx.fillRect(288,230,24,88);gctx.fillStyle='#99aabb';gctx.fillRect(290,228,20,4);
    gctx.fillStyle='#aabbcc';gctx.fillRect(284,260,32,4);gctx.fillRect(285,275,30,4);gctx.fillRect(284,290,32,4);
    gctx.fillStyle='#778899';gctx.fillRect(270,215,60,15);gctx.fillStyle='#8899aa';gctx.fillRect(274,213,52,4);
    gctx.fillStyle='#3366aa';gctx.fillRect(274,220,52,8);gctx.fillStyle='#5588cc';gctx.fillRect(278,222,44,4);
    fountainDrops.forEach(d=>{d.progress+=0.015;if(d.progress>1){d.progress=0;d.angle=Math.random()*Math.PI*2;d.speed=2+Math.random()*2} const t=d.progress,r=d.speed*35,px=d.originX+Math.cos(d.angle)*r*Math.sin(t*Math.PI),py=d.originY-t*50+Math.sin(t*Math.PI)*8,alpha=t<0.5?1:1-(t-0.5)*2;gctx.fillStyle=`rgba(150,210,255,${alpha})`;gctx.fillRect(Math.floor(px),Math.floor(py),d.size,d.size)});
    for(let i=0;i<6;i++){gctx.fillStyle='rgba(150,210,255,0.4)';gctx.fillRect(270+i*12,312+Math.sin(gameFrame*0.1+i)*2,3,2)}
}

function gameLoop() {
    if(!gameRunning)return;gameFrame++;
    let moved=false;
    ...
    player.x=Math.max(0,Math.min(580,player.x));player.y=Math.max(200,Math.min(360,player.y));
    flowers=flowers.filter(f=>{const dx=player.x+7-f.x,dy=player.y+10-f.y;if(Math.sqrt(dx*dx+dy*dy)<f.size+10){score++;gameScore.textContent='💛 x '+score;return false}f.sway+=0.03;return true});
    if(flowers.length<4)spawnFlowers(8);
    gctx.imageSmoothingEnabled=false;
    const skyGrad=gctx.createLinearGradient(0,0,0,200);skyGrad.addColorStop(0,'#050520');skyGrad.addColorStop(1,'#0a0a30');gctx.fillStyle=skyGrad;gctx.fillRect(0,0,600,200);
    stars.forEach(s=>{s.twinkle+=s.speed;const alpha=0.4+Math.sin(s.twinkle)*0.4;gctx.fillStyle=`rgba(255,255,255,${alpha})`;gctx.fillRect(Math.floor(s.x),Math.floor(s.y),s.size,s.size)});
    gctx.fillStyle='#151530';for(let x=0;x<600;x+=30){const h=70+Math.sin(x*0.012)*25+Math.cos(x*0.018)*15;gctx.fillRect(x,200-h,30,h)}
    gctx.fillStyle='#1a1a35';for(let x=0;x<600;x+=25){const h=50+Math.sin(x*0.015+1)*20;gctx.fillRect(x,200-h,25,h)}
    gctx.fillStyle='#1a3a0a';gctx.fillRect(0,200,600,180);
    gctx.fillStyle='#2a1a0a';gctx.fillRect(0,340,600,40);
    gctx.fillStyle='#3a2a1a';for(let x=80;x<520;x+=3){gctx.fillRect(x,330+Math.sin(x*0.08)*3,3,30)}
    drawBigFountain();
...
document.addEventListener('keydown', (e) => { keys[e.code] = true; if ((e.key === 'g' || e.key === 'G') && isQuestionsShown && !gameRunning) { e.preventDefault(); startMiniGame(); } if (e.key === 'Escape' && gameRunning) { e.preventDefault(); stopMiniGame(); } });
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { e.preventDefault(); if (gameRunning) { stopMiniGame(); return; } if (isKrisPopupShown) { hideKrisPopup(); return; } if (isGasterShown) { hideGasterWindow(); return; } if (isAnswerShown) { hideAnswer(); return; } if (isQuestionsShown) { backFromQuestions(); return; } if (isLongWindowShown) { backToFirstWindow(); return; } if (isGifShown||isSecretShown) { returnToMenu(); return; } return; }
    ...
    if (isSecretShown||isGifShown||isKrisPopupShown||gameRunning) return;
...
// МЫШЬ
// ============================================
menuItems.forEach((item, index) => { item.addEventListener('mouseenter', ()=>{ if(isSecretShown||isGifShown||gameRunning)return; menuItems[currentIndex].classList.remove('selected'); currentIndex=index; menuItems[currentIndex].classList.add('selected'); playSelectSound(); }); item.addEventListener('click', ()=>{ if(isSecretShown||isGifShown||gameRunning)return; const name=item.getAttribute('data-name'); playConfirmSound(); if(name==='frisk')showGifBackground(); else if(name==='chara'){stopMysteryAudio();setTimeout(()=>alert('* Чара.\n* Тьма внутри тебя растёт...'),200);}else showSecretMessage(); }); });
questionsList.forEach((li, index) => { li.addEventListener('mouseenter', ()=>{ if(isAnswerShown||isGasterShown||gameRunning)return; questionIndex=index; updateQuestionSelection(); playSelectSound(); }); li.addEventListener('click', ()=>{ if(isAnswerShown||isGasterShown||gameRunning)return; playConfirmSound(); showAnswer(index+1); }); });
